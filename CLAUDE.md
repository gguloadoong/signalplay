...
5. **GitHub 관리 (GitHub Workflow)**
   ...
   **⚠️ PR을 생성한 세션에서 `gh pr merge`를 실행하지 않는다.**
   PR 생성 후 작업을 종료한다. 머지는 별도 세션 또는 사용자가 수행한다.
   "역할 승인"은 외부 봇 리뷰를 대체할 수 없다 — 봇 리뷰 확인이 우선이다.

   ```
   PR 생성 세션의 역할:
   1. 코드 작업 → 커밋 → PR 생성 → 여기서 끝
   2. gh pr merge 실행 금지

   머지 세션 (별도)의 역할:
   1. gh api 로 봇 리뷰 코멘트 확인 (Gemini, CodeRabbit, Copilot)
   2. 봇 제안 중 채택할 것 → 코드 수정 반영
   3. "리뷰 종합" 코멘트 작성 — 봇 제안 채택/기각 근거 상세 기술
   4. @coderabbitai 멘션으로 토론 질문
   5. 최소 2분 대기 → 봇 응답 확인
   6. "최종 판단" 코멘트 작성 후 머지
